...
export default class BookmarkletFilter extends Filter {
  ...
  Mute tab: mute just one profanity word (Webfilter: Audio Bleep)
  _____ edit English subtitles with this _____
    ...
    if (!Page.isForbiddenNode(mutation.target)) {
      if (this.mutePage) {
        ...
        if (supported !== false && rule.simpleUnmute) {
          ...
          // Check for *** a filtered subtitle (no parent)
  ...
  cleanPage() {
    ...
    this.cfg.muteMethod = Constants.MuteMethods.Video; // Bookmarklet: Force video volume bleeped/muted method
    ...
    // Detect if we should mute/bleeped audio for the current page
